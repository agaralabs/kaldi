...
if [ "$data_dir" == "test_aspire" ]; then
  ...
  if [ $stage -le 1 ]; then
    ...
    steps/make_mfcc.sh --nj 1 --cmd "$train_cmd" --mfcc-config conf/mfcc_hires.conf $unseg_dir exp/make_mfcc_reverb/${data_dir}_whole $mfccdir || exit 1;
...
if [ $stage -le 6 ]; then
  ...
  local/multi_condition/decode.sh --nj 1 --cmd "$decode_cmd" --config conf/decode.config \
    ...
    data/graph data/${segmented_data_dir}_hires ${decode_dir}_stage1 || exit 1;
fi
